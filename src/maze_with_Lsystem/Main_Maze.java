...
public class Main_Maze extends JFrame implements MouseListener{
	...
	public void run(){
		try {
			...
			if(outputLogFile){
				outFile = new FileWriter("log_MaxNodes="+max_node_count+"_sight="+sight+"_"+Maze.width+"x"+Maze.height+
		...
		if(drawing == false){
			try {
				...
				CalcFractalDimension cf = 
						new CalcFractalDimension(result_dir+ID+"_search="+Simulator2D.draw_searchMAP+".png",
	...
	public static void main(String[] args) {		
		...
		Maze sp = new Maze(maze_file, startPoint, goalPoint, 6); //ただの壁あり
		// Maze sp = new Maze(maze_file, startPoint, goalPoint, checkPoint, 6); //中間地点あり
		// Maze sp = new Maze(maze_file, maze_file_detail, startPoint, goalPoint, 6); //山あり
		// Maze sp = new Maze(maze_file, maze_file_detail, startPoint, goalPoint, checkPoint, 6); //山あり 中間あり
		...
		// スタート位置はここで決めてる（変えるべき）
		MazeNode node = new MazeNode("0", "2", null, startPoint,0);
		// MazeNode node = new MazeNode("0", "2", null, new Point(start_point_x,start_point_y),0);
		...
		Main_Maze main = new Main_Maze(node, max_node_size, sight, logFile, goal_num, close_step, frame_rate,drawing,calc_fractalDimension, result_dir, pa, pb, pc);
